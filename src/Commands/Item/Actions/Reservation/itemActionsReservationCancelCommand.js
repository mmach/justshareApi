...
export default class ItemActionsReservationCancelCommand extends BaseCommand {
  /**
  ...
  async action() {
    try {
      ...
      let status = await this.statusProjectServiceDI.setContext(this.context).getByToken({ name: StatusesList.CANCELLED })
      ...
      await this.itemServiceDI.setContext(this.context).removeCategoryOptionTerm({
        model: {
          iua_id: IUA.id,
